...
bool IPrologueEvent::Update()
{
	...
	if(mCurrentTime > 2.f) TextOut(hdc, 500, 500, mEndRequest.c_str(), mEndRequest.size());

	if (mCurrentTime > 2.f and Input::GetInstance()->GetKeyUp(VK_LBUTTON)) 
	{
		return true;
	}
	return false;
